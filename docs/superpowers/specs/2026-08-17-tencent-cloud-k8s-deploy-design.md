...
- 2026-08-18：环境拆分三层结构——`overlays/subpath`（子路径中间层）、`overlays/qa`（测试：外部自建 PG、qa-ai）、`overlays/prod`（线上：TencentDB PG、prod-ai-lomva、域名占位）、`overlays/local`（kind + 唯一引用 incluster-postgres 组件）；namespace 对象随 overlay 走；中间件默认外置，自建 PG 仅为可选组件
