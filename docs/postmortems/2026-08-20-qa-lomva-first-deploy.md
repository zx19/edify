...
|---|---|---|
| 环境准备 | ~15s | |
| COPY 上下文 | ~33s | monorepo 根上下文 |
| pnpm build（next build） | ~137s | Turbopack 编译 112s + 写 next 文件缓存 21s |
| pnpm build:vinext | ~77s | 5 轮 vite 打包；部署不用 vinext（entrypoint 默认 targets/next，仅 EXPERIMENTAL_ENABLE_VINEXT=true 才用） |
| standalone 生成 | ~9s | |
